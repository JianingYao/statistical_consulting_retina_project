...
```{r}
cols_ind <- c(2, 4:112)
min_data <- apply(df[, cols_ind], 2, function(x) min(x, na.rm = TRUE))
max_data <- apply(df[, cols_ind], 2, function(x) max(x, na.rm = TRUE))
...
for (col in indep_list) {
  ...
  anova_results[[col]] <- aov(my_formula, data = df)
}
